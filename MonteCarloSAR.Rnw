...
eigenW = eigen( mW, symmetric = TRUE, only.values = FALSE, EISPACK = FALSE)

eigenWrange = 1 / range( eigenW$values ); eigenWrange

phi.low = eigenWrange[1] 
phi.hgh = eigenWrange[2] 

opt_mc = list()
opt_mc$ini_iter = 2000
opt_mc$max_iter = opt_mc$ini_iter + 10000
opt_mc$mh_scale = 0.035 
opt_mc$autotune = FALSE #  TRUE # 
opt_mc$target_accept_MH = 0.33

mh_scale = opt_mc$mh_scale

mc = list()
mc$runif = log( runif( opt_mc$max_iter ) )
mc$rnorm = rnorm( opt_mc$max_iter * ( dim( dfYX )[2] - 1 ) ) %>% matrix( ncol = opt_mc$max_iter )
mc$jump  = rnorm( opt_mc$max_iter ) * mh_scale

nobs = dim( dfYX )[1]; nobs
nreg = dim( dfYX )[2] - 1; nreg
...
mc_list = vector("list", length = opt_mc$max_it )
...
autotune_list = vector("list", length = opt_mc$max_it / 100 )
### START LOOP 
for( iter in 2:opt_mc$max_iter ){ # iter = 8
    ...
    sPhiP = sPhi + mc$jump[iter]
    if( sPhiP > phi.hgh | sPhiP < phi.low ){
      sLogDP = -Inf
    }else{
      mTemp = diag( dim( y )[1] ) - mW * sPhiP
    ...
    if( sLogDP - sLogD > mc$runif[iter] ){
      sPhi    = sPhiP
      sLogD   = sLogDP
      sAccept = 1
    }else{
      mXSXInv      = mc_iter$mXSXInv
      vBetaPhiHat  = mc_iter$vBetaPhiHat
      sSPhi2       = mc_iter$sSPhi2  
      sAccept      = 0
    }
...
length( mc_list )
mc_list       = list.remove( mc_list      , seq( 1, opt_mc$ini_iter) )
autotune_list = list.remove( autotune_list, seq( 1, opt_mc$ini_iter / 100 ) )
length( mc_list )

### COLLECT THE RESULTS
sPhi    = lapply( mc_list, `[[`, "sPhi"    ) %>% unlist
sLogD   = lapply( mc_list, `[[`, "sLogD"   ) %>% unlist
sAccept = lapply( mc_list, `[[`, "sAccept" ) %>% unlist
mBetaPhiHat = do.call( "cbind", lapply( mc_list, `[[`, "vBetaPhiHat" ) )
mBeta       = do.call( "cbind", lapply( mc_list, `[[`, "vBeta" ) )
sSigma2     = lapply( mc_list, `[[`, "sSigma2" ) %>% unlist


sAccept_rate = lapply( autotune_list, `[[`, "sAccept_rate"    ) %>% unlist
mh_scale     = lapply( autotune_list, `[[`, "mh_scale"    ) %>% unlist
